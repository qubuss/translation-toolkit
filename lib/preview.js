...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; }
    ...
    tr.fuzzy-row td.cell-key .fuzzy-badge {
      display: inline-block;
      font-size: 10px;
      font-weight: 600;
      color: #d97706;
  ...
  <script>
    ...
    function renderTranslations() {
      ...
      DATA.forEach((row, idx) => {
        ...
        const isFz = row.isFuzzy;
        const trClass = isPl && isFz ? 'plural-row fuzzy-row' : isPl ? 'plural-row' : isFz ? 'fuzzy-row' : '';
        html += '<tr' + (trClass ? ' class="' + trClass + '"' : '') + '>';
        html += '<td class="cell-key" style="color:#999;font-size:12px">' + shown + '</td>';
        html += '<td class="cell-key">' + highlightText(row.key, searchEl.value.trim()) + (isPl ? ' <span class="plural-badge">plural</span>' : '') + (isFz ? ' <span class="fuzzy-badge">fuzzy</span>' : '') + '</td>';
        visibleLangs.forEach(lang => {
    ...
    function csvToData(content) {
      ...
      // Detect and skip _status column (added by export v1.6.0+)
      const hasStatusCol = (rows[0][1] || '').trim() === '_status';
      const langStart = hasStatusCol ? 2 : 1;
      const langs = rows[0].slice(langStart).map(l => l.trim());
      const dataMap = new Map();
      for (let i = 1; i < rows.length; i++) {
        const f = rows[i]; const key = (f[0] || '').trim(); if (!key) continue;
        const trans = {};
        for (let j = 0; j < langs.length; j++) trans[langs[j]] = (f[j+langStart] || '').trim();
...
async function startPreview(translationsDir, port, watchMode = false) {
  function loadAll() {
    ...
    const rows = [...allKeys].map((rawKey) => {
      ...
      const isFuzzy = allFuzzyKeys.has(rawKey);
      return isFuzzy ? { key: displayKey, translations, isFuzzy: true } : { key: displayKey, translations };
    });
    ...
    for (const rawKey of allPluralKeys) {
      ...
      for (let n = 0; n < maxForms; n++) {
        ...
        rows.push(allFuzzyKeys.has(rawKey)
          ? { key: displayKey + '[' + n + ']', translations, isPlural: true, pluralForm: n, isFuzzy: true }
          : { key: displayKey + '[' + n + ']', translations, isPlural: true, pluralForm: n });
      }
    }
      ...
      req.on('end', () => {
        try {
          ...
          function parseCsvString(content) {
            ...
            // Detect and skip _status column
            const hasStatusCol = headerFields[1] === '_status';
            const langStart = hasStatusCol ? 2 : 1;
            const languages = headerFields.slice(langStart).map(l => l.trim());
            const rows = new Map();
            for (let i = 1; i < lines.length; i++) {
              ...
              for (let j = 0; j < languages.length; j++) {
                translations[languages[j]] = (fields[j + langStart] || '').trim();
              }
  ...
  function tryListen(tryPort) {
    server.once('error', (err) => {
      if (err.code === 'EADDRINUSE') {
        ...
        console.error(`  Port ${tryPort} is in use, trying ${nextPort}...`);
    ...
    server.listen(tryPort, () => {
      ...
      console.error(`  → http://localhost:${tryPort}\n`);
      if (tryPort !== port) {
        console.error(`  (requested port ${port} was in use)`);
      }
      console.error(`  ${state.rows.length} keys × ${state.languages.length} languages (${state.languages.join(', ')})`);
      console.error(`  Validation: ${issueCount === 0 ? '✓ no issues' : issueCount + ' issues found'}`);
      if (watchMode) {
        console.error(`  Watch mode: ON — auto-reloads when .po files change`);
      }
