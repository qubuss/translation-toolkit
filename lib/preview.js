 ...
 * @returns {string}
 */
function buildHtml(rows, languages, translationsDir, validationResult, statsResult) {
  ...
  <div class="tab-panel" id="panel-diff">
    <div class="diff-upload-zone" id="diffUploadZone">
      <div class="diff-mode-toggle">
        <button class="diff-mode-btn active" data-mode="csv-vs-po">CSV vs Current .po</button>
        <button class="diff-mode-btn" data-mode="csv-vs-csv">CSV vs CSV</button>
      </div>
  ...
  <script>
    ...
    function renderTranslations() {
      ...
      DATA.forEach((row, idx) => {
        ...
        html += '<td class="cell-key">' + highlightText(row.key, searchEl.value.trim()) + '</td>';
        visibleLangs.forEach(lang => {
 ...
 * @param {number} port
 */
async function startPreview(translationsDir, port) {
  ...
  // Map short code to file path for saving
      ...
      req.on('end', () => {
        try {
          ...
          for (const [lang, keyValues] of Object.entries(changes)) {
            const poFile = shortCodeToFile.get(lang);
            if (!poFile) continue;
...
async function runPreview(args) {
  ...
  await startPreview(translationsDir, port);
}
