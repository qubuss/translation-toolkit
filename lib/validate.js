...
function validateTranslations(translationsDir) {
  ...
  for (const poFile of poFiles) {
    const { entries, pluralEntries, header } = parsePo(poFile.filePath);
  ...
  return { issues, refLang, languages, totalKeys: refEntries.size, totalPluralKeys: refPluralEntries.size };
}
...
function printReport(result) {
  const { issues, refLang, languages, totalKeys, totalPluralKeys } = result;

  console.log();
  console.log(`${C.bold}translation-toolkit validate${C.reset}`);
  const pluralInfo = totalPluralKeys ? ` + ${totalPluralKeys} plural` : '';
  console.log(`${C.dim}Reference language: ${refLang} (${totalKeys} keys${pluralInfo})${C.reset}`);
