...
/**
 * Parse a .po file into an object { header, entries, pluralEntries, fuzzyKeys }.
 ...
 * - fuzzyKeys: Set of keys that have the #, fuzzy flag
 *
 * @param {string} filePath - absolute path to .po file
 * @returns {{ header: string[], entries: Map<string, string>, pluralEntries: Map<string, PluralEntry>, fuzzyKeys: Set<string> }}
 */
function parsePo(filePath) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Skip comments (but detect #, fuzzy flag)
    if (line.startsWith('#')) {
  ...
  return { header: headerLines, entries, pluralEntries, fuzzyKeys };
}
...
/**
 * Modify a comment buffer to add/remove the `fuzzy` flag.
 *
 * @param {string[]} commentLines - buffered comment/blank lines before an entry
 * @param {boolean} shouldBeFuzzy - true = ensure fuzzy flag present; false = remove it
 * @returns {string[]} modified comment lines
 */
function _applyFuzzyChange(commentLines, shouldBeFuzzy) {
  const result = [];
  let hasFuzzyFlag = false;
  ...
  for (const line of commentLines) {
    if (line.startsWith('#,') && /\bfuzzy\b/.test(line)) {
      ...
      if (shouldBeFuzzy) {
        ...
        // Remove fuzzy from the flags line, keep other flags
        const flags = line.slice(2).split(',').map(f => f.trim()).filter(f => f && f !== 'fuzzy');
        if (flags.length > 0) {
          result.push('#, ' + flags.join(', '));
        }
        // else: line was only "#, fuzzy" — drop it entirely
      }
    } else {
      result.push(line);
    }
  ...
  // If should be fuzzy but no flag found, add #, fuzzy before the entry
  if (shouldBeFuzzy && !hasFuzzyFlag) {
    result.push('#, fuzzy');
  }
 ...
 * @param {Map<string, boolean>} [fuzzyChanges] - key → true (add fuzzy) / false (remove fuzzy)
 */
function patchPoFile(filePath, newEntries, removeAbsent = false, newPluralEntries = new Map(), fuzzyChanges = new Map()) {
  const content = fs.readFileSync(filePath, 'utf-8');
  const lines = content.split('\n');
  const output = [];
  ...
  // Buffer for comment/blank lines preceding an entry — allows modifying #, fuzzy
  let commentBuffer = [];

  while (i < lines.length) {
    const line = lines[i];

    // Blank or comment line — buffer (don't push yet)
    ...
    if (!entry) {
      // Unrecognized line — flush buffer and preserve
      output.push(...commentBuffer);
      commentBuffer = [];
      output.push(line);
      i++;
      continue;
    ...
    // Apply fuzzy changes to comment buffer (skip header)
    if (!isHeader && fuzzyChanges.has(compositeKey)) {
      commentBuffer = _applyFuzzyChange(commentBuffer, fuzzyChanges.get(compositeKey));
    }
  ...
  // Flush any trailing comments/blank lines
  output.push(...commentBuffer);
  commentBuffer = [];
