...
const { exportToJson } = require('./jsonFormat');
const { exportToI18next } = require('./i18nextFormat');
...
async function runExport(args) {
  ...
  if (format === 'json') {
    const dir = outputDir || '.';
    exportToJson(dir, translationsDir, { includeStatus });
  } else if (format === 'i18next') {
    const dir = outputDir || '.';
    exportToI18next(dir, translationsDir, { compatibilityJSON: compatJSON });
  } else {
