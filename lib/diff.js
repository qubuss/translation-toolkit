...
function parseCsvFile(csvPath, delimiter) {
  ...
  const langStartIdx = hasStatusCol ? 2 : 1;
  const languages = headerFields.slice(langStartIdx); // skip 'key' (and _status if present)
  ...
  for (let i = 1; i < csvRows.length; i++) {
    ...
    for (let j = 0; j < languages.length; j++) {
      translations[languages[j]] = fields[j + langStartIdx] || '';
    }
